...
func TestLoaderRejectsAmbiguousName(t *testing.T) {
	index := []SkillEntry{
	...
	if !strings.Contains(err.Error(), "AMBIGUOUS_SKILL") {
		t.Fatalf("ambiguous name error = %v, want AMBIGUOUS_SKILL", err)
	}
	for _, sensitive := range []string{"/a/SKILL.md", "/b/SKILL.md", "codex", "claude"} {
		if strings.Contains(err.Error(), sensitive) {
			t.Fatalf("ambiguous name error leaked source detail %q: %v", sensitive, err)
		}
	}
}
